...
export default function Contact({ userRef, listItems }) {
  const [landlord, setLandlord] = useState(null);
  const [message, setMessage] = useState("");
  useEffect(() => {
    async function getLandlord() {
      const docRef = doc(fdb, "users", userRef);
    ...
    <>
      {landlord !== null && (
        <div className="flex flex-col w-full">
          <p>
            Contact {landlord.name} for the {listItems.name.toLowerCase()}
          </p>
          ...
          <a
            href={`mailto:${landlord.email}?Subject=${listItems.name}&body=${message}`}
          >
            <button
